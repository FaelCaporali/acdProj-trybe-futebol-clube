...
import authMid from '../shared/auth/middleware'
...
describe("Tests for private routes access and general errors treatments", async () => {
...
describe("", () => {
  ...
  it("Should have a basic error handler on public and private routes, responding to all services to the error message if provided, and the 500 status", async () => {
    sinon.stub(authMid.prototype, 'registerRequest').callsFake(middlewareSkipper);
    sinon.stub(authMid.prototype, 'logRequest').callsFake(middlewareSkipper);
